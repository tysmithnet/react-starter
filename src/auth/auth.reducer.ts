...
function handleLoginRequest(state: IRootState, action: ILoginRequest): IRootState {
    return {
        ...state,
    };
}

function handleLoginSuccess(state: IRootState, action: ILoginSuccess): IRootState {
    return {
        ...state,
        user: action.user,
    };
